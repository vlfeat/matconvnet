    ...
    case 1
      ...
      for weighed = [false true]
        for multilab = [false true]
          ...
          vl_testder(@(x) vl_nnsoftmaxloss(x,c), x, dzdy, dzdx, 1e-3) ;
          ...
          dzdy = grandn(size(y)) ;
          dzdx = vl_nnsoftmaxloss(x,c,dzdy) ;
          vl_testder(@(x) vl_nnsoftmaxloss(x,c), x, dzdy, dzdx, range * 1e-3) ;
        end
      end
        ...
        for c=[2 8 17]
          for nd=[1 3 4]
            for bs=[2 7]
              ...
              y = vl_nnbnorm(x,g,b,'verbose') ;
              dzdy = grandn(size(y), 'single') ;
              [dzdx,dzdg,dzdb] = vl_nnbnorm(x,g,b,dzdy,'verbose') ;
      
              vl_testder(@(x) vl_nnbnorm(x,g,b), x, dzdy, dzdx, range * 1e-2, -1e-3) ;
              vl_testder(@(g) vl_nnbnorm(x,g,b), g, dzdy, dzdg, range * 1e-2, -1e-3) ;
              vl_testder(@(b) vl_nnbnorm(x,g,b), b, dzdy, dzdb, range * 1e-2, -1e-3) ;
            end
          end
        end
      end
