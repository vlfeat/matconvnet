...
% for cluttered MNIST:
function nn = init_cluttered_STN(opts)
	% init the object:
	nn = dagnn.DagNN();

	% ************************** localization network ****************************
	l_mp1 = dagnn.Pooling('method', 'max', 'poolSize', [2 2],'pad', 0, 'stride', 2);
	nn.addLayer('l_mp1', l_mp1, {'img'}, {'x1'});
	...
	% ************************** spatial transformer ******************************
	aff_grid = dagnn.AffineGridGenerator('Ho',opts.imsz(1),'Wo',opts.imsz(2));
	nn.addLayer('aff', aff_grid,{'aff'},{'grid'});

	sampler = dagnn.BilinearSampler();
	nn.addLayer('samp',sampler,{'img','grid'},{'xST'});
	% *****************************************************************************
	...
	% softmax loss:
	nn.addLayer('err', dagnn.Loss('loss', 'softmaxlog'), {'pred','label'}, 'err');
    nn.addLayer('err_show', dagnn.Loss('loss', 'classerror'), {'pred','label'}, 'err_show');
	% *****************************************************************************
	
	nn.initParams();
	% bias the transformation to IDENTITY:
