...
for epoch=1:opts.numEpochs
  ...
  for t=1:opts.batchSize:numel(train)
    ...
    for l=1:numel(net.layers)
      ...
      net.layers{l}.filtersMomentum = ...
        opts.momentum * net.layers{l}.filtersMomentum ...
          ...
          - (lr * net.layers{l}.filtersLearningRate) / numel(batch) * dzdw{l}{1} ;

      net.layers{l}.biasesMomentum = ...
        opts.momentum * net.layers{l}.biasesMomentum ...
          - (lr * net.layers{l}.biasesLearningRate) * ....
          (opts.weightDecay * net.layers{l}.biasesWeightDecay) * net.layers{l}.biases ...
          - (lr * net.layers{l}.biasesLearningRate) / numel(batch) * dzdw{l}{2} ;
    ...
    info.train = updateError(opts, info.train, res, labels, batch_time) ;
  ...
  for t=1:opts.batchSize:numel(val)
    ...
    input = struct('name', {'data', 'label'}, 'x', {im, labels});
    res = vl_dagnn(net, input, [], res, 'disableDropout', true, ...
      'sync', opts.sync) ;

    % print information
    batch_time = toc(batch_time) ;
    speed = numel(batch)/batch_time ;
    info.val = updateError(opts, info.val, res, labels, batch_time) ;
...
% -------------------------------------------------------------------------
function info = updateError(opts, info, res, labels, speed)
% -------------------------------------------------------------------------
