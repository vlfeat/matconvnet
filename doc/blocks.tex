...
The \emph{convolution transpose} block (some authors call it improperly deconvolution) implements an operator this is the transpose of the convolution described in Sectio\ref{s:convolution}n~. Let
\[
 \bx\in\real^{H \times W \times D}, \quad
 \bff\in\real^{H' \times W' \times D \times D''}, \quad
...
\]
$(S_h,S_w)$ are the vertical and horizontal \emph{upsampling factors},  $(P_h^-,P_h^+,P_h^-,P_h^+)$ the \emph{output crops}, and $\bx$ and $\bff$ are zero-padded as needed in the calculation. Furthermore, given the input and filter heights, the output height is given by
\[
  H'' = S_h H + H' -P^-_h - P^+_h.
\]
