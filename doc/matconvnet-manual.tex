...
There are other libraries which implement the CNN architecture. The most notable frameworks implemented in \cpp are Cuda-Convnet, the original CNN implementation by \cite{krizhevsky12imagenet} and Caffe~\cite{jia13caffe}, which inspired internals of \vlnn. Caffe has a Matlab interface, however the network definitions are written in custom language. A powerful Torch7 toolbox \cite{collobert2011torch7}, implemented in Lua, aims for creating a general machine learning framework and has implementation for all CNN building blocks. Another framework, Theano \cite{bergstra2010}, is a Python library which directly solves mathematical expressions and allows to define CNN architectures. Both of these libraries can perform computations on CPU and GPU.
Among Matlab toolboxes, there is a DeepLearningToolbox \cite{deepltbx12} which implements all algorithms in pure Matlab which has its speed limitations.
